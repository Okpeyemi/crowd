      ...
      <div className="px-4 py-14 md:px-6 md:py-20">
        ...
        <div className="mt-10 grid gap-5 md:mt-12 md:grid-cols-2 lg:grid-cols-4">
          {cards.map((c, i) => (
            <div
              key={i}
              className="rounded-2xl bg-[#D2AEF5] p-6 text-center backdrop-blur-sm shadow-sm border border-white/30"
            >
              <div className="flex items-center justify-center">
                <Image src={c.icon} alt="icon" width={80} height={80} />
              </div>
              <p className="mt-6 text-sm font-medium text-[#0B1131] md:text-base">
                {c.text}
              </p>
            </div>
          ))}
